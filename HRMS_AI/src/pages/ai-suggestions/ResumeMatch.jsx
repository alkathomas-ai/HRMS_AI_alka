...
const renderSimilarProfilesTab = () => (
    <div className="similar-profiles-page">
      ...
      <div className="similar-profiles-left">
        ...
        <div className="sp-upload-card">
          ...
          <p className="sp-upload-hint">Find employees with similar skills and experience</p>
          ...
          <button className="btn-primary" style={{ width: '100%', justifyContent: 'center' }} onClick={handleResumeUpload} disabled={!resumeFile || resumeUploading}>
            {resumeUploading
              ? <><div className="spinner-small"></div>Analyzing...</>
              : <><span className="material-symbols-outlined" style={{ fontSize: 16 }}>manage_search</span>Find Similar Profiles</>}
          </button>
        </div>
        ...
        {similarProfiles && !resumeUploading && (
          <div className="sp-summary-card">
            <div className="sp-summary-header">
              ...
              <div>
                <div className="sp-summary-designation">{similarProfiles.resume_profile.designation}</div>
                <div className="sp-summary-chips">
                  <span className="resume-info-chip primary">{similarProfiles.resume_profile.seniority}</span>
                  <span className="resume-info-chip">{similarProfiles.resume_profile.domain}</span>
                  <span className="resume-info-chip">{similarProfiles.resume_profile.total_experience_years} yrs</span>
                </div>
              </div>
            </div>
            ...
            <div className="sp-skills-section">
              ...
              <div className="sp-skills-list">
                {similarProfiles.resume_profile.primary_skills.map((s, i) => (
                  <span key={i} className="skill-badge primary-skill-badge">{s}</span>
                ))}
              </div>
            </div>
            <div className="sp-skills-section">
              ...
              <table className="projects-table">
                ...
                <tbody>
                  {similarProfiles.similar_profiles.map((emp) => {
                    const scoreClass = emp.ai_score >= 70 ? 'high' : emp.ai_score >= 50 ? 'medium' : 'low';
                    return (
                      <tr key={emp.employee_id} className="project-row">
                        <td>
                          <div className="project-name-cell">
                            <div className="sp-emp-avatar">{emp.display_name?.charAt(0).toUpperCase()}</div>
                            <div>
                              <div style={{ fontWeight: 600, fontSize: 13 }}>{emp.display_name}</div>
                              <div style={{ fontSize: 11, color: 'var(--color-text-secondary)' }}>{emp.employee_id}</div>
                            </div>
                          </div>
                        </td>
                        ...
                        <td>{emp.total_exp}</td>
                        <td>
                          <span className={`status-badge ${emp.availability_pct > 0 ? 'started' : 'cancelled'}`}>
                            {emp.availability_pct > 0 ? `${emp.availability_pct}% free` : 'Engaged'}
                          </span>
                        </td>
                        <td><span className={`similar-score-badge ${scoreClass}`}>{emp.ai_score}</span></td>
                        <td>
                          <div className="table-skills">
                            {emp.primary_skills.slice(0, 3).map((s, i) => <span key={i} className="skill-badge">{s}</span>)}
                            {emp.primary_skills.length > 3 && <span className="skill-badge">+{emp.primary_skills.length - 3}</span>}
                          </div>
                        </td>
                      </tr>
                    );
    ...
    return (
        <div className="resume-match-section">
            ...
            <button onClick={handleUpload} disabled={resumeUploading}>
