      ...
      <div className="suggestion-row" onClick={() => setExpanded(!expanded)}>
        <div className={`suggestion-score-circle ${expanded ? 'expanded' : ''}`}>
          <svg className="score-progress" viewBox="0 0 36 36">
            ...
            <path className={`score-fill ${scoreClass}`} strokeDasharray={`${ai_score}, 100`} d="M18 2.0845 a 15.9155 15.9155 0 0 1 0 31.831 a 15.9155 15.9155 0 0 1 0 -31.831" />
          </svg>
          ...
          {!expanded && (
            <div className="suggestion-skills-preview">
              {skill_set?.split(',').slice(0, 3).map((s, i) => (
                <span key={i} className="skill-badge-preview">{s.trim()}</span>
              ))}
              {skill_set?.split(',').length > 3 && (
                <span className="skill-badge-preview more">+{skill_set.split(',').length - 3}</span>
              )}
            </div>
          )}
            ...
            <div className="skills-section">
              <div className="skills-container">
                {skill_set.split(',').slice(0, showAllSkills ? undefined : 6).map((skill, i) => {
                  ...
                  return (
                    <span
                      ...
                      className={isActive ? 'skill-badge-preview active-skill-badge-preview' : 'skill-badge-preview'}
                    >
                      {trimmed}
                    </span>
                  );
                ...
                {skill_set.split(',').length > 6 && (
                  <button onClick={(e) => { e.stopPropagation(); setShowAllSkills(!showAllSkills); }} className="skill-more-btn-preview">
